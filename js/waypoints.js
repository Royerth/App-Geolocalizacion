...
window.onload = function () {
    loadGoogleMaps("places", function () {
        var latlng = new google.maps.LatLng(-17.38425230806001, -66.13495099450682);
        var options = {
            zoom: 6,
            center: latlng,
...
function buildPoints() {
    ...
    for (var i = 0; i < points.length; i++) {
        ...
        markers.push(marker);
        marker.setMap(map);
        html += "<tr><td>" + points[i].LocationName + "</td><td><input name=\"lats[]\" value=\"" + roundNumber(points[i].LatLng.lat(), 6) + "\" /></td><td><input name=\"lngs[]\" value=\"" + roundNumber(points[i].LatLng.lng(), 6) + "\" /></td><td><button class=\"delete btn\" onclick=\"removeRow(" + i + ");\">Eliminar</button></td><td></tr>";
        // if (i < points.length - 1) {
        //     html += "<button class=\"moveDown btn\" onclick=\"moveRowDown(" + i + ");\">Subir</button>";
        // }
        // html += "</td><td>";
        // if (i > 0) {
        //     html += "<button class=\"moveUp btn\" onclick=\"moveRowUp(" + i + ");\">Bajar</button>";
        // }
        // html += "</td></tr>";
    }
